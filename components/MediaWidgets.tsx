...
export async function YouTubeReviewWidget() {
    const ytVideo: YouTubeVideo = await fetchLatestYouTube();

    return (
        <a href={ytVideo.url} target="_blank" rel="noopener noreferrer" className="block w-full">
            <section className="bg-zinc-900/80 rounded-2xl overflow-hidden border border-zinc-800 flex flex-col group cursor-pointer hover:border-zinc-700 transition-all">
...
export async function LiveShippingWidget() {
    const cafePosts: CafePost[] = await fetchLiveShipping();

    return (
        <section className="bg-zinc-900/80 rounded-2xl overflow-hidden border border-zinc-800 flex flex-col hover:border-zinc-700 transition-all">
